...
ReadBC_tower aReadBC_tower ;

// #pragma GCC diagnostic push
// #pragma GCC diagnostic ignored "-Wshadow"

ReadBC_tower::ReadBC_tower() : Processor("ReadBC_tower"),
			     m_colNameBCal(""),
			     m_nameOutputFile(""),
  ...
  _description = "ReadBC_tower reads the simulation of BeamCal and creates two std::vector<double> of energy deposits in the largest tower and its neighbors for each event." ;
  ...
  registerProcessorParameter ("Threshold",
			      "Energy threshold for accepting pads.",
			      m_threshold,
			      double(0.001) ) ;
  ...
  // 			      m_pdftitle,
  // 			      std::string("ReadBC_tower.pdf") ) ;


}
//#pragma GCC diagnostic pop

void ReadBC_tower::init() {
...
void ReadBC_tower::processRunHeader( LCRunHeader* ) {
  //  streamlog_out (DEBUG) << "Runnumber "<< _nRun << std::endl;
  //   if(_nRun % 4 == 0) {
}

void ReadBC_tower::processEvent( LCEvent * evt ) {
...
void ReadBC_tower::check( LCEvent * ) {
  // nothing to check here - could be used to fill checkplots in reconstruction processor
}


void ReadBC_tower::end(){
